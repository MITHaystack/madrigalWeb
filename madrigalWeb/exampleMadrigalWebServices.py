...
result = testData.downloadFile(
    thisFilename, "/tmp/test.txt", user_fullname, user_email, user_affiliation, "simple"
)
result = testData.downloadFile(
    thisFilename, "/tmp/test.hdf5", user_fullname, user_email, user_affiliation, "simple"
)
...
result = testData.madCalculator(
    1999, 2, 15, 12, 30, 0, 45, 55, 5, -170, -150, 10, 200, 200, 0, "sdwht,kp,ne_iri"
)
for line in result:
    for value in line:
        print(("%8.2e " % (value)))
...
print(
    "Since this experiment is not local (note the experiment id = -1), we need to create a new MadrigalData object to get it"
)
