...
"""globalCitation.py is a script that runs a global search through Madrigal data, and returns a citation to the group of files.
...
parser.add_argument(
    "--dateList",
    help="comma separated list of date strings in the form YYYY-MM-DD, to get experiments for \
                                a list of discrete days. Must include startDate and endDate. ",
)
...
if args.dateList:
    args.dateList = [
        datetime.datetime.strptime(item.strip(), "%Y-%m-%d") for item in args.dateList.split(",")
    ]
...
citationList = server.getCitationListFromFilters(
    startDate,
    endDate,
    inst,
    kindat,
...
citation = server.createCitationGroupFromList(
    citationList, user_fullname, user_email, user_affiliation
)
