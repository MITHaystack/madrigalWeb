#!python
"""globalDownload.py is a script that runs a global search through Madrigal data from a given URL, and downloads all matching files.
...
Examples
--------
>>>globalDownload.py --url=https://cedar.openmadrigal.org --outputDir=/tmp/tmpcvr52632 --user_fullname=CI_Test --user_email=citest@example.com --user_affiliation=Test --format=hdf5 --startDate=01/19/1998 --endDate=01/21/1998 --inst=30 --verbose
"""
...
def getInstrumentList(inst, server):
    ...
    inst : str
        ...
        the argument must be enclosed in double quotes.  An asterisk will perform matching as in glob.
...
def filterExperimentsUsingExpName(expList, expName):
    ...
    expList : list of MadrigalExperiment objects
        a list of MadrigalExperiment objects to be filtered
    expName  : str
        filter experiments by the experiment name.  Can be all or part of the experiment name. Matching
        is case insensitive.
...
def excludeExperimentsUsingExpName(expList, expName):
    ...
    expList : list of MadrigalExperiment objects
        a list of MadrigalExperiment objects to be filtered
    expName  : str
        exclude experiments by the experiment name.  Can be all or part of the experiment name. Matching
...
def getExperimentFileList(server, expList, includeNonDefault, verbose=False):
    ...
    server : MadrigalData object
        the active MadrigalData object to get information from
    expList : list of MadrigalExperiment
        the list of desired MadrigalExperiment objects
    includeNonDefault : int
        1 if should include non-default files, 0 otherwise
...
def filterExperimentFilesUsingFileDesc(expFileList, fileDesc):
    ...
    expFileList : list of MadrigalExperimentFile objects
        a list of MadrigalExperimentFile objects to be filtered.
    fileDesc : str
        string to search for in the experiment files' file descriptions
...
def getSubdirectoryFromFullFile(fullFilename):
    ...
    -------
    str
        everything after experiments* and before filename
    """
...
parser.add_argument(
    "--outputDir", type=str, required=True, help="Path to a valid, writable output directory."
)
parser.add_argument(
...
parser.add_argument(
    "--tree",
    action="store_true",
    help="add if you want to store the downloaded files in the same hierarchy as in Madrigal: \
   <YYYY/<instCode>/<experimentDir>.  Without --tree, stores all downloaded files in one directory.",
)
parser.add_argument(
...
for i in range(numFiles):
    ...
    if os.access(saveName, os.R_OK):
        ...
        while True:
            newName = (
                saveName[0 : saveName.rfind(".")]
                + "_%i" % (count - 1)
                + saveName[saveName.rfind(".") :]
            )
    ...
    try:
        server.downloadFile(
            expFileList[i].name, saveName, user_fullname, user_email, user_affiliation, format
        )
        ...
        print(
            (
                "Failure downloading %s because it took more than allowed number of seconds"
                % (expFileList[i].name)
            )
        )
